"""Unit tests for proofloop.suite (catalog-driven test-suite construction).

Ported from the ship book's gmstest suite; all open-path (no GMS / no GPU).
Exercises catalogs, resolution + applies_to filtering, the open base sources,
cross/embedded composition, SFT emitters, and the open evaluate consumer
(MockSUT, run/summary, and the weak_link attribution fallback).
"""
from __future__ import annotations

import pytest

from proofloop import suite as gt
from proofloop.suite.evaluate import MockSUT, SUTResult, run, summary, weak_link
